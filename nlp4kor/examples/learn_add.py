...
def input_pipeline(filenames, batch_size=100, shuffle=False, delim='\t', tokens=2):  # TODO: multi reader
                ...
                with tf.Session(config=config) as sess:
                    ...
                    if reuse_model and checkpoint and checkpoint.model_checkpoint_path:  # predicting
                        ...
                        try:
                            ...
                            log.info('')
                            log.info('with queue runner: %.2f secs (batch_size: %s)' % (watch.elapsed(), batch_size))
                            log.info('')
                        except:
                            log.info(traceback.format_exc())
                        finally:
