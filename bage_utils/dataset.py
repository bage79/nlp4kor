...
class DataSet(object):
    def __init__(self, features: np.ndarray, labels: np.ndarray, features_vector: OneHotVector, labels_vector: OneHotVector):
        """
    ...
    def __repr__(self):
        return '%s (feature: %s, label:%s, size: %s)' % (self.__class__.__name__, type(self.features[0]), type(self.labels[0]), self.size)
